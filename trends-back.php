...
<head>
    ...
    <script>
        ...
        function refreshQuestions(category, target) {
            target = '#question'+target.substring(target.length-1);
            $(target).val('');
        ...
        function addQuestion() {
            numQuestions++;
            var category = $('#category1').clone();
            category.attr('id', 'category'+numQuestions);
            category.appendTo('#extraQuestions');
            //onchange needs to be changed to point to new question

            var question = $('<select id="question1" style="width:300px" class="searchbox" style="margin-bottom: 10px"> \
                <option value="" selected="selected">Select a question</option> \
            </select><br>');
            question.attr('id', 'question'+numQuestions);
            question.appendTo('#extraQuestions');
            refreshQuestions('','#question'+numQuestions);
...
<div class="container" id="main">
    <div class="row" style="background-color: #2e6da4;">
        <div class="searchbar">
            <label class="shadow">1. Select a question:</label>
            <select id="category1" style="width:160px; margin-bottom: 10px" class="selector" onchange="refreshQuestions(this.value, this.id)">
            ...
            <label class="shadow">2. (Optional) Add additional questions to the graph:</label>
            <input type="button" value="Add Question" id="addQuestion" class="btn" onclick="addQuestion()" style="margin-top:-20px">
            <div id="extraQuestions" style="margin-left: 224px"></div>
