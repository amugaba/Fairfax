<footer class="footer" style="z-index: 1000; position: absolute; bottom: 0; width:100%; background: #d6d6d6; padding: 10px">
    <div style="margin: 0 auto; text-align: center; max-width: 500px;">
        <div style="float: left; margin-right: 50px">
            <img src="img/fcps_logo.png">
        </div>
        <div style="float: left; margin-right: 50px">
            <a href="index.php">Home</a> | <a href="category.php">Highlights</a> | <a href="graphs.php">Explore</a><br>
            <span style="font-size: 10pt"><a href="contact.php">Contact</a> | <a href="http://www.fairfaxcounty.gov/youthsurvey" target="_blank">Fairfax Survey Home</a></span><br><br>
            <span class="text-muted" style="font-size: 10pt">&copy; 2016 - Fairfax County, Virginia</span>
        </div>
        <div style="float: left">
            <img src="img/fairfax_county_logo.png">

        </div>
    </div>
</footer>
</div>